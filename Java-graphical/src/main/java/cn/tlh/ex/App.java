...
import cn.tlh.ex.util.POITLStyle;
import com.deepoove.poi.XWPFTemplate;
import com.deepoove.poi.data.DocxRenderData;
import com.deepoove.poi.data.MiniTableRenderData;
import com.deepoove.poi.data.RowRenderData;
import com.deepoove.poi.data.TextRenderData;
import cn.tlh.ex.util.FileUtils;
...
public class App {
    public static void main(String[] args) throws IOException {
        ...
        if (map.get("-t") == null || "mysql".equals(map.get("-t"))) {
            mySql(map);
        } else if ("oracle".equals(map.get("-t"))) {
            oracle(map);
        } else {
            System.out.println("-t=mysql or oracle defalut mysql");
            System.exit(0);
    ...
    public static void oracle(Map<String, String> map) throws IOException {
        ...
        String oracleSql1 =
                "select " +
                        ...
                        "ut.tablespace_name as engine," +
                        "ut.buffer_pool as table_collation," +
                        " uc.table_type as table_type," +
                        "uc.comments as table_comment," +
                        "ut.last_analyzed as create_options " +
                        "from user_tables ut,user_tab_comments uc where ut.table_name=uc.table_name";
        String oracleSql2 =
                "select rownum as ordinal_position," +
                        ...
                        "from user_col_comments t,user_tab_columns c where c.column_name=t.column_name and c.table_name=t.table_name and t.table_name='";
        ResultSet rs = OracleUtils.getResultSet(Objects.requireNonNull(OracleUtils.getConnnection(String.format("jdbc:oracle:thin:@%s:%s:ORCL", map.get("h"), map.get("p")), map.get("-u"), map.get("-p"))), oracleSql1);
    ...
    public static void mySql(Map<String, String> map) throws IOException {
        ...
        String mysqlSql1 =
                "SELECT " +
                        ...
                        "table_comment, " +
                        "create_options " +
                        "FROM information_schema.TABLES WHERE table_schema='" + map.get("-n") + "'";
        //查询表的结构信息
        String mysqlSql2 =
                "SELECT " +
                        "ordinal_position," +
        ...
        ResultSet rs = MySqlUtils.getResultSet(Objects.requireNonNull(MySqlUtils.getConnnection(String.format("jdbc:mysql://%s:%s", map.get("h"), map.get("p")), map.get("-u"), map.get("-p"))), mysqlSql1);
        Connection con = MySqlUtils.getConnnection(String.format("jdbc:mysql://%s:%s", map.get("h"), map.get("p")), map.get("-u"), map.get("-p"));
    ...
    private static void createDoc(ResultSet rs, String sqls, Map<String, String> map, String outFile, boolean type, String title, Connection con) throws IOException {
        ...
        for (Map<String, String> str : list) {
            ...
            ResultSet set = MySqlUtils.getResultSet(con, sql);
